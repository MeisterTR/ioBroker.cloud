    ...
    this.process = function (request, isEnabled, callback) {
        ...
        switch (request.header.name) {
            ...
            case 'TurnOffRequest': {
                    ...
                    for (let channel in channels) {
                        let toggle = findRoleInChannel(channels[channel], 'switch');
                         adapter.log.debug(device.additionalApplianceDetails.group);
                        adapter.log.debug(device.additionalApplianceDetails.byONs[toggle]);
                        adapter.log.debug(device.additionalApplianceDetails.byON);
                        if (toggle) {
                            device.additionalApplianceDetails.group
                            let byon = (device.additionalApplianceDetails.group) ?
                                            device.additionalApplianceDetails.byONs[toggle] :
                                            device.additionalApplianceDetails.byON;
                        ...
                        if (!toggle) toggle = findRoleInChannel(channels[channel], 'level.dimmer');
